...
export const createFogPlane = () => {
    ...
    return new Mesh( new PlaneGeometry(3, 3, 700, 394), mat);
}
...
{
    return 100.0 * (iMouse.y / vUv.y);
}

float GetRotationFactor()
{
    return iMouse.x / vUv.x;
}
...
void main()
{
    ...
    vec3 color3 = Render(rayOrigin, rayDirection);
    gl_FragColor=vec4( GammaCorrect(clamp(color3, 0.0, 1.0)), 1.0 );
}
`